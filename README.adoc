...
TIP: css selector *$=* denotes to _'endsWith'_ and is really helpful when working with JSF _prepended_ id strategy.
...
The message *Welcome to Primefaces!!* is shown in growl component in the following format (again firebug is your best friend, use firebug tab *response*):
...
IMPORTANT: Gatling response is a bit different then firebug (in this case) and that's because Gatling *DOES NOT* execute javascript contained in responses (that's a browser thing)
[source,xml]
.Gatling response
----
<partial-response id="j_id1"><changes><update id="j_idt87:growl"><![CDATA[<span id="j_idt87:growl" class="ui-growl-pl"
data-widget="widget_j_idt87_growl" data-summary="data-summary" data-severity="all,error" data-redisplay="true"></span>
<script id="j_idt87:growl_s" type="text/javascript">$(function(){PrimeFaces.cw('Growl','widget_j_idt87_growl',
{id:'j_idt87:growl',sticky:false,life:2000,escape:true,msgs:[]});});</script>]]></update>
<update id="j_id1:javax.faces.ViewState:0"><![CDATA[5739601299644801453:-7228355357233819622]]></update>
